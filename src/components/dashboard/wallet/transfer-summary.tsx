    ...
    <div className="w-full max-w-xl rounded-[2.5rem] bg-gradient-to-b from-[#1C1C27] to-[#14141B] p-6">
      ...
      <div className="text-center mb-8">
        <div className="mb-2">
          ...
          <p className="text-lg text-gray-400 mt-1">TO {recipient.toUpperCase()}</p>
        </div>

        {quote && (
          <div className="bg-[#2F2F3A] rounded-xl p-3 mt-4">
            <div className="flex items-center justify-center gap-2 mb-2">
              <TrendingUp className="h-4 w-4 text-green-500" />
              <span className="text-sm text-white">You&apos;ll send: {quote.tokenAmount.toFixed(6)} {token}</span>
            </div>
            <p className="text-xs text-gray-400">Rate: ₦{quote.rate.toLocaleString()} per {token}</p>
          </div>
        )}
      </div>

      {/* Transaction Details */}
      <div className="bg-gradient-to-b from-[#1C1C27]/80 to-[#14141B]/60 rounded-3xl overflow-hidden mb-6">
        <div className="space-y-4 p-6">
          {/* Transfer Type */}
          <div className="flex justify-between items-center">
            <span className="text-base text-white">Transfer type</span>
          ...
          <div className="flex justify-between">
            <span className="text-base text-white">Recipient Details</span>
            <div className="text-right">
              <p className="text-sm text-gray-400">{recipient}</p>
              <p className="text-sm text-gray-400">{accountNumber}</p>
              <p className="text-sm text-gray-400">{bankName}</p>
            </div>
          </div>
          ...
          {quote && (
            <div className="flex justify-between">
              <span className="text-base text-white">Token Details</span>
              <div className="text-right">
                <p className="text-sm text-gray-400">{quote.tokenAmount.toFixed(6)} {token}</p>
          ...
          {/* Smart Contract ID */}
          <div className="flex justify-between">
            <span className="text-base text-white">Smart Contract</span>
            <span className="text-sm text-gray-400 font-mono">PayDirect v2.0</span>
          </div>
          ...
          <div className="border-t border-gray-600 pt-4">
            <div className="flex justify-between mb-2">
              <span className="text-base text-white">Subtotal</span>
              <span className="text-sm text-gray-400">₦{amount.toLocaleString()}</span>
            </div>
            <div className="flex justify-between mb-2">
              <span className="text-base text-white">Platform Fee (1%)</span>
              <div className="text-right">
                <p className="text-sm text-gray-400">₦{fiatFee.toFixed(2)}</p>
                {currentTokenPrice > 0 ? (
            ...
            <div className="flex justify-between font-medium">
              ...
              <span className="text-lg text-white">₦{totalAmount.toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-4 mb-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm text-purple-400 font-medium mb-1">Secure Transaction</p>
      ...
      <div className="space-y-4">
        ...
        {!approved && (
          <button
            ...
            className={`w-full rounded-2xl py-4 text-lg font-medium transition-all duration-200 ${approving || loading
        ...
        {approved && (
          <button
            onClick={onConfirm}
            disabled={loading}
            className={`w-full rounded-2xl py-4 text-lg font-medium transition-all duration-200 ${loading
              ? "bg-gray-600 cursor-not-allowed text-gray-400"
              : "bg-gradient-to-r from-purple-600 to-purple-500 hover:from-purple-700 hover:to-purple-600 text-white shadow-lg hover:shadow-purple-500/25"
              }`}
          >
        ...
        {approved && !loading && (
          <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-green-400 font-medium">Tokens approved successfully</span>
            </div>
          </div>
        )}
      </div>
